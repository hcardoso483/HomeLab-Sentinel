...
def apply_migrations(connection):
    ...
    for version, path in migrations:
        ...
        info(f"Applying inventory migration {current} -> {version}: {path.name}")
...
def store_observation(connection, record):
    ...
    existing = connection.execute(
        """
        SELECT monitoring_observation_id
    ...
    observation_id = f"mon-{uuid.uuid4().hex}"
    connection.execute(
        """
...
def main():
    parser = argparse.ArgumentParser(
        description="HomeLab Sentinel Monitoring Observation Store"
    )
    parser.add_argument(
        "--database",
        type=Path,
        ...
        try:
            ...
            if version < 3:
                raise ValueError(
                    ...
                    "does not support Monitoring observations"
                )
            stored, duplicates = process_stream(connection)
        finally:
            connection.close()
    except (OSError, sqlite3.Error, ValueError, KeyError) as exc:
        error(f"Monitoring observation store failed: {exc}")
        return 1

    info(
        f"Monitoring observation store complete. "
